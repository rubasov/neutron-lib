...
class TestPlacementUtils(base.BaseTestCase):
    ...
    def test_traitify(self):
        self.assertEqual('SOMESTRING', place_utils.traitify('somestring'))
        self.assertEqual('SOME_STRING', place_utils.traitify('some-string'))
        self.assertEqual('SOME_STRING', place_utils.traitify('SOME_STRING'))
    ...
    def test_parse_rp_bandwidths(self):
        ...
        # NOTE(bence romsics): This contradicts the spec, but IMHO prohibiting
        # this format makes the config option syntax clearer. See also:
        # https://review.openstack.org/#/c/508149/14/specs/rocky
        #        /minimum-bandwidth-allocation-placement-api.rst@159
        self.assertRaises(
            ValueError,
            place_utils.parse_rp_bandwidths,
            ['eth0'],
